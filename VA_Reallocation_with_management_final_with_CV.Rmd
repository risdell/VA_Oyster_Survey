...
# Plot the change in quality of the data using the CV with each sequential reduction in effort
Piankatank.plot.dat.cv <- Piankatank.plot.dat %>% 
  group_by(KMEAN, ER) %>% 
  summarise(CV = cv(MEAN))
## Create a sequence of the no effort reduction CV for comparison
Piankatank.plot.dat.cv$PctCV <- Piankatank.plot.dat.cv$CV / rep(pull(Piankatank.plot.dat.cv[c(seq(1,((k-1)*10)+1,10)),3]), each = 10) * 100 - 100

ggplot(Piankatank.plot.dat.cv, aes(y = PctCV, x = ER, color = KMEAN, linetype = KMEAN)) +
  geom_line(size = 1.2) +
  geom_abline(intercept = 40, slope = 0, size = 1.2, linetype = "dashed") +
  theme_light() + 
  scale_color_brewer(palette = "Paired") +
...
# Plot the change in quality of the data using the CV with each sequential reduction in effort
York.plot.dat.cv <- York.plot.dat %>% 
  group_by(Subgroup, ER) %>% 
  summarise(CV = cv(MEAN))
## Create a sequence of the no effort reduction CV for comparison
York.plot.dat.cv$PctCV <- York.plot.dat.cv$CV / rep(pull(York.plot.dat.cv[c(seq(1,((k-1)*10)+1,10)),3]), each = 10) * 100 - 100

ggplot(York.plot.dat.cv, aes(y = PctCV, x = ER, color = Subgroup, linetype = Subgroup)) +
  geom_line(size = 1.2) +
  geom_abline(intercept = 40, slope = 0, size = 1.2, linetype = "dashed") +
  theme_light() + 
  scale_color_brewer(palette = "Paired") +
...
# Plot the change in quality of the data using the CV with each sequential reduction in effort
Great_Wicomico.plot.dat.cv <- Great_Wicomico.plot.dat %>% 
  group_by(Subgroup, ER) %>% 
  summarise(CV = cv(MEAN))
## Create a sequence of the no effort reduction CV for comparison
Great_Wicomico.plot.dat.cv$PctCV <- Great_Wicomico.plot.dat.cv$CV / rep(pull(Great_Wicomico.plot.dat.cv[c(seq(1,((k-1)*10)+1,10)),3]), each = 10) * 100 - 100

ggplot(Great_Wicomico.plot.dat.cv, aes(y = PctCV, x = ER, color = Subgroup, linetype = Subgroup)) +
  geom_line(size = 1.2) +
  geom_abline(intercept = 40, slope = 0, size = 1.2, linetype = "dashed") +
  theme_light() + 
  scale_color_brewer(palette = "Paired") +
...
# Plot the change in quality of the data using the CV with each sequential reduction in effort
Chesapeake.plot.dat.cv <- Chesapeake.plot.dat %>% 
  group_by(Subgroup, ER) %>% 
  summarise(CV = cv(MEAN))
## Create a sequence of the no effort reduction CV for comparison
Chesapeake.plot.dat.cv$PctCV <- Chesapeake.plot.dat.cv$CV / rep(pull(Chesapeake.plot.dat.cv[c(seq(1,((k-1)*10)+1,10)),3]), each = 10) * 100 - 100

ggplot(Chesapeake.plot.dat.cv, aes(y = PctCV, x = ER, color = Subgroup, linetype = Subgroup)) +
  geom_line(size = 1.2) +
  geom_abline(intercept = 40, slope = 0, size = 1.2, linetype = "dashed") +
  theme_light() + 
  scale_color_brewer(palette = "Paired") +
...
# Plot the change in quality of the data using the CV with each sequential reduction in effort
Tangier.plot.dat.cv <- Tangier.plot.dat %>% 
  group_by(Subgroup, ER) %>% 
  summarise(CV = cv(MEAN))
## Create a sequence of the no effort reduction CV for comparison
Tangier.plot.dat.cv$PctCV <- Tangier.plot.dat.cv$CV / rep(pull(Tangier.plot.dat.cv[c(seq(1,((k-1)*10)+1,10)),3]), each = 10) * 100 - 100

ggplot(Tangier.plot.dat.cv, aes(y = PctCV, x = ER, color = Subgroup, linetype = Subgroup)) +
  geom_line(size = 1.2) +
  geom_abline(intercept = 40, slope = 0, size = 1.2, linetype = "dashed") +
  theme_light() + 
  scale_color_brewer(palette = "Paired") +
